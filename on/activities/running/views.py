...
def get_days(goal):
    ...
    for i in range(day_index):
        ...
        last = start + timedelta(days=i)
        last_end = last + timedelta(days=1)
        # 从那天开始到那一天结束的时间
        end = last_date + timedelta(days=1)
        is_no_sign_in = False
        punch_inform = None
        # punchs = goal.punch.filter(record_time__range=(last-timedelta(hours=8), last_end-timedelta(hours=8)))
